...
对于 DNS 解析和 TCP 连接，前端可以做的努力非常有限。相比之下，HTTP 连接这一层面的优化才是我们网络优化的核心。
...
这两个优化点直直地指向了我们日常开发中非常常见的操作——资源的压缩与合并。没错，这就是我们每天用构建工具 webpack 在做的事情。
:::
...
module: {
  rules: [
    {
      test: /\.js$/,
      exclude: /(node_modules|bower_components)/, // 用 include 或 exclude 来帮我们避免不必要的转译
      use: {
...
> Tree shaking is a term commonly used in the html context for dead-code elimination, or more precisely, live-code import. It relies on ES2015 module import/export for the static structure of its module system.

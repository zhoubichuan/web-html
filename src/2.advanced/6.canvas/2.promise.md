...
<script>
  ...
  function clipImage() {
    ...
    canvas.width = width2*4
    canvas.height = height2*4
    // 在canvas上绘制图片
    if(image.naturalWidth/image.naturalHeight>width2/height2){
      ctx.drawImage(image, 0, 0,image.naturalWidth/image.naturalHeight*height2*4,height2*4)
      }else{
      ctx.drawImage(image, 0, 0,width2*4,image.naturalHeight/image.naturalWidth/width2*4)
    }
    // 将剪裁后的图片转换为DataURL
